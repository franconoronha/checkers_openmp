...
Board minimax(Board b, int depth, bool maximizingPlayer, int alpha, int beta, bool flag) {
    if (depth == 0 || b.isGameOver()) {
        return b;
    }
    Board bestMove;
    if (maximizingPlayer) {
        ...
        for (int i = 0; i < size; ++i) {
            /* if (flag) continue; */
            Board newBoard = minimax(moves[i], depth - 1, false, alpha, beta, false);
            int score = newBoard.evaluate();
            #pragma critical
            {
                if (score > bestScore) {
        ...
        for (int i = 0; i < size; ++i) {
            /* if (flag) continue; */
            Board newBoard = minimax(moves[i], depth - 1, true, alpha, beta, false);
            int score = newBoard.evaluate();
            #pragma critical
            {
                if (score < bestScore) {
    ...
    while (!isGameOver())
    {
        ...
        if (turn == WHITE_TURN) {
            ...
            #pragma omp parallel
            {
                #pragma omp single
                {
                    Board bestMove = minimax(*this, max_depth, true, -1000, 1000, flag);
                }
            }
