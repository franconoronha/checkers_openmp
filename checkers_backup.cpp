...
Board minimax(Board b, int depth, bool maximizingPlayer, int alpha, int beta) {
    ...
    if (maximizingPlayer) {
        vector<Board> moves = b.findMovesAndCaptures();
        Board bestMove, bestMove2, bestMove3, bestMove4;
        int bestScore = -1000;
        int bestScore2 = -1000;
        int bestScore3 = -1000;
        int bestScore4 = -1000;
        int size = moves.size();
        int chunk = size / 4;
        int lastChunk = chunk + size % 4;
                #pragma omp task shared(bestMove, bestScore, alpha, beta)
                ...
                #pragma omp task shared(bestMove2, bestScore2, alpha2, beta2)
                for (int i = chunk; i < chunk * 2; ++i) {
                    Board newBoard = minimax(moves[i], depth - 1, false, alpha2, beta2);
                    int score2 = newBoard.evaluate();
                ...
                #pragma omp task shared(bestMove3, bestScore3, alpha3, beta3)
                for (int i = chunk * 2; i < chunk * 3; ++i) {
                    Board newBoard = minimax(moves[i], depth - 1, false, alpha3, beta3);
                    int score3 = newBoard.evaluate();
                ...
                #pragma omp task shared(bestMove4, bestScore4, alpha4, beta4)
                for (int i = chunk * 3; i < lastChunk; ++i) {
                    Board newBoard = minimax(moves[i], depth - 1, false, alpha4, beta4);
                    int score4 = newBoard.evaluate();
                ...
                #pragma omp taskwait
                if (bestScore > bestScore2 && bestScore > bestScore3 && bestScore > bestScore4) {
                    return bestMove;
                } else if (bestScore2 > bestScore && bestScore2 > bestScore3 && bestScore2 > bestScore4) {
                    return bestMove2;
                } else if (bestScore3 > bestScore && bestScore3 > bestScore2 && bestScore3 > bestScore4) {
                    return bestMove3;
                } else {
                    return bestMove4;
                }
        ...
        int lastChunk = chunk + size % 4;
                ...
                #pragma omp task shared(bestMove2, bestScore2, alpha2, beta2)
                for (int i = chunk; i < chunk * 2; ++i) {
                    Board newBoard = minimax(moves[i], depth - 1, true, alpha2, beta2);
                    int score2 = newBoard.evaluate();
                    if (score2 > bestScore2) {
                        bestScore2 = score2;
                        bestMove2 = moves[i];
                ...
                #pragma omp task shared(bestMove3, bestScore3, alpha3, beta3)
                for (int i = chunk * 2; i < chunk * 3; ++i) {
                    Board newBoard = minimax(moves[i], depth - 1, true, alpha3, beta3);
                    int score3 = newBoard.evaluate();
                    if (score3 > bestScore3) {
                        bestScore3 = score3;
                        bestMove3 = moves[i];
                ...
                #pragma omp task shared(bestMove4, bestScore4, alpha4, beta4)
                for (int i = chunk * 3; i < lastChunk; ++i) {
                    Board newBoard = minimax(moves[i], depth - 1, true, alpha4, beta4);
                    int score4 = newBoard.evaluate();
                    if (score4 > bestScore4) {
                        bestScore4 = score4;
                        bestMove4 = moves[i];
